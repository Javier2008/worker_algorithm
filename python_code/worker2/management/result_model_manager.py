...
from common.common import base_utils
from common.common.file_utils import get_project_base_directory
from common.common.log import getLogger
from worker2 import Locker, local_cache_required
from worker2.config.settings import TEMP_DIRECTORY, job_component_name, job_component_module_name
from worker2.management.model_manage import ModelManage

LOGGER = getLogger()


class ResultModelManager(Locker):
    def __init__(self, model_id, model_version):
        os.makedirs(TEMP_DIRECTORY, exist_ok=True)
        
        self.model_id = model_id
        self.model_version = model_version
        self.model_path = get_project_base_directory("model_local_cache", model_id, model_version)
        ...
        self.pipeline_model_name = "job"
        self.pipeline_model_alias = "Job"
    ...
    def create_job_model(self):
        ...
        with self.lock:
            ...
            os.makedirs(os.path.join(self.model_path, "define"))
    ...
    def save_job_model(self, pipeline_buffer_object):
        model_buffers = {self.pipeline_model_name: (
        type(pipeline_buffer_object).__name__, pipeline_buffer_object.SerializeToString(),
        json_format.MessageToDict(pipeline_buffer_object, including_default_value_fields=True))}
        self.save_component_model(component_name=job_component_name(),
                                  component_module_name=job_component_module_name(),
    ...
    def create_component_model(self, component_name, component_module_name, model_alias,
        ...
        for model_name, (proto_index, object_serialized, object_json) in model_buffers.items():
            ...
            component_model["buffer"][storage_path.replace(get_project_base_directory(), "")] = (
            base64.b64encode(object_serialized).decode(), object_json)
            model_proto_index[model_name] = proto_index  # index of model name and proto buffer class name
            LOGGER.info("save {} {} {} buffer".format(component_name, model_alias, model_name))
    ...
    def write_component_model(self, component_model):
        for storage_path, (object_serialized_encoded, object_json) in component_model.get("buffer").items():
            storage_path = get_project_base_directory() + storage_path
        ...
        LOGGER.info("save {} {} successfully".format(component_model["component_name"],
    ...
    def read_job_model(self, parse=True):
        # todo: integration with read_component_model
        component_name = job_component_name()
        model_alias = self.pipeline_model_alias
        component_model_storage_path = os.path.join(self.variables_data_path, component_name, model_alias)
        ...
        for model_name, buffer_name in model_proto_index.items():
            with open(os.path.join(component_model_storage_path, model_name), "rb") as fr:
                ...
                if parse:
                    ...
                    model_buffers[model_name] = [buffer_name,
    ...
    def packaging_model(self):
        archive_file_path = shutil.make_archive(base_name=self.archive_model_base_path,
                                                format=self.default_archive_format, root_dir=self.model_path)
        ...
        LOGGER.info("Make model {} {} archive on {} successfully. sha1: {}".format(
    ...
    def unpack_model(self, archive_file_path: str):
        ...
        LOGGER.info("Unpack model archive to {}".format(self.model_path))
    ...
    def update_component_meta(self, component_name, component_module_name, model_alias, model_proto_index):
        ...
        with self.lock, open(self.define_meta_path, "r+", encoding="utf-8") as f:
            ...
            define_index["model_proto"][component_name][model_alias] = define_index["model_proto"][component_name].get(
                model_alias, {})
    ...
    def reload_component_model(self, model_id, model_version, component_list):
        for component_name in component_list:
            ...
            source_pipeline_model = ModelManage(model_id, model_version)
            ...
            for model_alias, model_proto_index in component_model_proto.items():
                self.update_component_meta(component_name, component_define.get("module_name"), model_alias,
                                           model_proto_index)
