...
class HeteroLRTransferVariable(BaseTransferVariables):
    def __init__(self, flowid=0):
        ...
        self.begin_iter = self._create_variable(name='begin_iter', src=['guest'], dst=['host', 'arbiter'])
        self.batch_validate_info = self._create_variable(name="batch_validate_info", src=['host'], dst=['guest'])
        self.converge_flag = self._create_variable(name='converge_flag', src=['guest'], dst=['host'])
        ...
        self.host_gradient = self._create_variable(name='host_gradient', src=['guest'], dst=['host'])
        self.host_en_gradient_r = self._create_variable(name='provider_en_gradient_r', src=['host'], dst=['guest'])
        ...
        self.host_weight = self._create_variable(name='provider_weight', src=['host'], dst=['guest'])
