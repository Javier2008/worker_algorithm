...
from algorithm.components.components import ComponentMeta
from algorithm.model_base import MetricMeta, ModelBase
from algorithm.param.base_param import BaseParam
from common import session
from common.abc import AddressABC, StorageTableABC, StorageTableMetaABC
from common.common import EngineType, log
from common.common import data_utils
from common.common._types import InputSearchType
from common.common.data_utils import default_output_fs_path, default_output_info
from common.computing import ComputingEngine
from common.session import Session
from common.storage import StorageEngine, StorageTableMeta, StorageTableOrigin
...
class Reader(ModelBase):
    ...
    def _run(self, cpn_input: TaskInput):
        ...
        input_table_namespace, input_table_name = self.get_input_table_info(
            ...
            role=self.tracker_client.role,
            party_id=self.tracker_client.party_id,
        )
        (
            output_table_namespace,
            output_table_name,
        ) = default_output_info(
            task_id=self.tracker_client.task_id,
            task_version=self.tracker_client.task_version,
            output_type="data",
        )
        (
            input_table_meta,
        ...
        # self.tracker_client.log_output_data_info(
        ...
        #         "job_id": self.tracker_client.job_id,
        #     },
        # )
    ...
    def get_input_table_info(parameters, role, party_id):
        ...
        if search_type is InputSearchType.TABLE_INFO:
            ...
            output_data_infos = ResultManagement.query_output_data_infos(
    ...
    def convert_check(
        ...
        output_name,
        output_namespace,
        computing_engine: ComputingEngine = ComputingEngine.SPARK,
        ...
        if input_table_meta.get_engine() in [StorageEngine.PATH]:
            from common.storage import PathStoreType
            ...
            from common.storage import StandaloneStoreType
    ...
    def to_save(self, src_table, dest_table):
        ...
        schema = src_table_meta.get_schema()
        self.tracker_client.save_output_data(
            src_computing_table,
        ...
        #Add by yuyunfu??
        self.data_output = src_computing_table
