...
from algorithm.framework.weights import Weights
...
def get_data_converter(config_type) -> DataConverter:
    if config_type == "pytorch":
        from algorithm.feature.nn.backend.pytorch.nn_model import PytorchDataConverter

        return PytorchDataConverter()
    else:
        from algorithm.feature.nn.backend.tf_keras.nn_model import KerasSequenceDataConverter
...
def get_nn_builder(config_type):
    if config_type == "nn":
        ...
        from algorithm.feature.nn.backend.tf_keras.layers import get_builder, has_builder
        ...
        from algorithm.feature.nn.backend.tf_keras.nn_model import build_keras

        return build_keras
    elif config_type == "pytorch":
        from algorithm.feature.nn.backend.pytorch.nn_model import build_pytorch
...
def restore_nn_model(config_type, model_bytes):
    if config_type == "nn" or config_type == "keras":
        ...
        from algorithm.feature.nn.backend.pytorch.nn_model import PytorchNNModel
