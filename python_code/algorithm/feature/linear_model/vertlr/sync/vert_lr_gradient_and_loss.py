...
from algorithm.feature.instance import Instance
from algorithm.feature.linear_model.vertlr.sync import loss_sync
...
from algorithm.optim.activation import sigmoid
from algorithm.statistic.data_overview import is_sparse_data
from algorithm.util import consts
from algorithm.util.fate_operator import _one_dimension_dot, vec_dot
...
def __compute_partition_gradient(data, fit_intercept=True, is_sparse=False):
    ...
    if is_sparse:
        ...
        gradient = cpu_dot(sparse_matrix.transpose(), fore_gradient).tolist()
        if fit_intercept:
        ...
        gradient = cpu_dot(feature.transpose(), fore_gradient)
...
class Promoter(VertGradientBase, loss_sync.Promoter):
    ...
    def register_gradient_procedure(self, transfer_variables):
        self._register_gradient_sync(transfer_variables.host_weight,
                                     transfer_variables.host_forward_dict,
                                     transfer_variables.fore_gradient,
                                     transfer_variables.host_gradient,
                                     transfer_variables.host_en_gradient_r)
...
class Provider(VertGradientBase, loss_sync.Provider):
    ...
    def remote_provider_forward(self, provider_forward, suffix=tuple()):
        self.provider_forward_transfer.remote(obj=provider_forward, role=consts.GUEST, idx=0, suffix=suffix)

    def remote_provider_weight(self, provider_weight):
        self.provider_weight_transfer.remote(obj=provider_weight, role=consts.GUEST, idx=0)
    ...
    def sync_gradient_r(self, e_gradient_r, suffix=tuple()):
        self.provider_en_gradient_r_transfer.remote(obj=e_gradient_r, role=consts.GUEST, idx=-1, suffix=suffix)
