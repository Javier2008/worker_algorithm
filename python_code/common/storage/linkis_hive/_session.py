...
from common.common.address import LinkisHiveAddress
from common.storage import StorageSessionBase, StorageEngine, LinkisHiveStoreType
...
class StorageSession(StorageSessionBase):
    ...
    def table(self, name, namespace, address: AddressABC, partitions,
        ...
        if isinstance(address, LinkisHiveAddress):
            from common.storage.linkis_hive._table import StorageTable
            return StorageTable(
                address=address,
                name=name,
                namespace=namespace,
                ...
                options=options)
        raise NotImplementedError(f"address type {type(address)} not supported  storage")
