...
from common.common import engine_utils, EngineType
from common.abc import CSessionABC, FederationABC, CTableABC, StorageSessionABC, StorageTableABC, StorageTableMetaABC
from common.common import log, base_utils
from common.common import remote_status
from common.computing import ComputingEngine
from common.federation import FederationEngine
from common.storage import StorageEngine, StorageSessionBase
from common.metastore.db_models import DB, SessionRecord, init_database_tables
from common.session._parties import PartiesInfo
...
class Session(object):
    ...
    def __init__(self, session_id: str = None, task_conf:dict =None):
        ...
        self._computing_type = task_conf.get(EngineType.COMPUTING, engines.get(EngineType.COMPUTING, None))
        self._federation_type = task_conf.get(EngineType.FEDERATION, engines.get(EngineType.FEDERATION, None))
        self._storage_engine = task_conf.get(EngineType.STORAGE, engines.get(EngineType.STORAGE, None))
    ...
    def init_computing(self,
        ...
        if self._computing_type == ComputingEngine.STANDALONE:
            from common.computing.standalone import CSession
        ...
        if self._computing_type == ComputingEngine.SPARK:
            from common.computing.spark import CSession
    ...
    def init_federation(
        ...
        if self._federation_type == FederationEngine.STANDALONE:
            from common.computing.standalone import CSession
            from common.federation.standalone import Federation
        ...
        if self._federation_type == FederationEngine.RABBITMQ:
            from common.computing.spark import CSession
            from common.federation.rabbitmq import Federation
        ...
        if self._federation_type == FederationEngine.PULSAR:
            from common.computing.spark import CSession
            from common.federation.pulsar import Federation
    ...
    def _get_or_create_storage(self,
        ...
        if storage_engine == StorageEngine.STANDALONE:
            from common.storage.standalone import StorageSession
            ...
            from common.storage.mysql import StorageSession
            storage_session = StorageSession(session_id=storage_session_id, options=kwargs.get("options", {}))
        
        elif storage_engine == StorageEngine.HDFS:
            from common.storage.hdfs import StorageSession
            storage_session = StorageSession(session_id=storage_session_id, options=kwargs.get("options", {}))
        
        elif storage_engine == StorageEngine.HIVE:
            from common.storage.hive import StorageSession
            storage_session = StorageSession(session_id=storage_session_id, options=kwargs.get("options", {}))
        
        elif storage_engine == StorageEngine.LINKIS_HIVE:
            from common.storage.linkis_hive import StorageSession
            storage_session = StorageSession(session_id=storage_session_id, options=kwargs.get("options", {}))
        
        elif storage_engine == StorageEngine.PATH:
            from common.storage.path import StorageSession
            storage_session = StorageSession(session_id=storage_session_id, options=kwargs.get("options", {}))
        
        elif storage_engine == StorageEngine.LOCALFS:
            from common.storage.localfs import StorageSession
            storage_session = StorageSession(session_id=storage_session_id, options=kwargs.get("options", {}))
