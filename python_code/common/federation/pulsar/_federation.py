...
from common._standalone import _create_table
from common.abc import FederationABC, GarbageCollectionABC
from common.common import Party
from common.common import file_utils
from common.common.log import getLogger
from common.computing.spark import Table
from common.computing.spark._materialize import materialize
from common.computing.standalone import Table as STable
from common.federation._datastream import Datastream
from common.federation.pulsar._mq_channel import (
    MQChannel,
    DEFAULT_TENANT,
    DEFAULT_CLUSTER,
    DEFAULT_SUBSCRIPTION_NAME,
)
from common.federation.pulsar._pulsar_manager import PulsarManager
...
class Federation(FederationABC):
    ...
    def get(
        ...
        elif dtype == FederationDataType.STable:
            party_topic_infos = self._get_party_topic_infos(
                parties, name, partitions=partitions
            )
            for i in range(len(party_topic_infos)):
                ...
                receive_func = self._get_receive_func(
                    name,
                    tag,
                    party_id,
                    role,
                ...
                _table = _create_table(
                    session=get_session()._computing_session.get_standalone_session(),
                    name=name,
                ...
                _table = _table.mapPartitions(receive_func)
                table = STable(_table)
    ...
    def remote(
        ...
        if _name_dtype_keys[0] not in self._name_dtype_map:
            ...
            elif isinstance(v, STable):
                body = {"dtype": FederationDataType.STable,
                        "partitions": v.partitions}
            else:
        ...
        if isinstance(v, Table):
            ...
            if re.search("paillier_pubkey", name) is not None:
                print("send" + str(v))
    ...
    def cleanup(self, parties):
        ...
        # 0, close all connection
        for k, v in self._channels_map.items():
            v.cancel()
            LOGGER.debug(f"successfully close channel {k}")
        ...
        # 4. clear all backlog ? no API, try to delete namespace
        response = self._pulsar_manager.delete_namespace(
            self._tenant, self._session_id, True
        )
        if response.ok:
            LOGGER.debug(f"successfully delete pulsar namespace {self._session_id}")
        else:
            LOGGER.error(f"delete pulsar namespace with response {response.reason} {response.text}")
    ...
    def _get_receive_func(
            self, name, tag, party_id, role, topic_infos, mq, conf: dict
    ):
        def _fn(kvs):
            return self._partition_receive(
                0, kvs, name, tag, party_id, role, topic_infos, mq, conf
            )
