...
__BIG_OBJ_MAX_PART_SIZE = "__big_obj_max_part_size"
...
def segment_transfer_enabled(max_part_size=None):
    ...
    return _attr_injected_meta_class(**{__BIG_OBJ_MAX_PART_SIZE: max_part_size})


def _is_splitable_obj(obj):
    return hasattr(obj, __BIG_OBJ_MAX_PART_SIZE)
...
def _get_splits(obj) -> typing.Tuple[typing.Any, typing.Iterable]:
    ...
    num_slice = (byte_size - 1) // getattr(obj, __BIG_OBJ_MAX_PART_SIZE) + 1
    if num_slice <= 1:
        return obj, ()
    else:
        head = _SplitHead(num_slice)
        _max_size = getattr(obj, __BIG_OBJ_MAX_PART_SIZE)
