...
class LoggerFactory(object):
    ...
    def get_handler(class_name, level=None, log_dir=None, log_type=None, job_id=None):
        if not log_type:
            ...
            log_file = os.path.join(log_dir, "flow_{}.log".format(
                log_type) if level == LoggerFactory.LEVEL else 'flow_{}_error.log'.format(log_type))
...
def setDirectory(directory=None):
    LoggerFactory.set_directory(directory, force=True)
