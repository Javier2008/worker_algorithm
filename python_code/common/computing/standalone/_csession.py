...
from common._standalone import Session
from common.abc import AddressABC, CSessionABC
from common.common.base_utils import gen_uuid
from common.common.log import getLogger
from common.computing.standalone._table import Table
...
class CSession(CSessionABC):
    ...
    def load(self, address: AddressABC, partitions: int, schema: dict, **kwargs):
        from common.common.address import StandaloneAddress
        ...
        if isinstance(address, StandaloneAddress):
            ...
            if address.storage_type != StandaloneStoreType.ROLLPAIR_IN_MEMORY:
                raw_table = raw_table.save_as(
                    name=f"{address.name}_{gen_uuid()}",
